...
if(cl.support.localStorage){clStorage = cl.localStorage();}
	...
	cl.fn.textShadow = function(option) {
		if (!cl.browser.msie) return;
		var IE6 = cl.browser.version < 7;
		return this.each(function() {
			var el = cl(this);
			var shadow = el.textShadowParse(this.currentStyle["text-shadow"]);
			shadow = cl.extend(shadow, option);
			...
			cl(span).addClass("jQueryTextShadow");
			cl(span).html(el.html());
			cl(span).css({
			...
			if (shadow.radius != 0) {
				if (shadow.opacity != null) {
					cl(span).css("filter", "progid:DXImageTransform.Microsoft.Blur(pixelradius="+parseInt(shadow.radius)+", enabled='true', makeShadow='true', ShadowOpacity="+shadow.opacity+")");
				} else {
					cl(span).css("filter", "progid:DXImageTransform.Microsoft.Blur(pixelradius="+parseInt(shadow.radius)+", enabled='true')");
				}
			}	
			el.append(span);
	...
	cl.fn.textShadowParse = function(value) 
	{
		value = String(value)
			.replace(/^\s+|\s+$/gi, '')
	...
	cl.fn.textShadowRemove = function() {
		if (!cl.browser.msie) return;
		return this.each(function() {
			cl(this).children("span.jQueryTextShadow").remove();
		});
	};
...
cl.fn.jmp3 = function(passedOptions){
	var playerpath = document.location.protocol+"//bhelp.com/core/";var options = {"filepath": "/static/","backcolor": "ffffff","forecolor": "ffffff","width": "0","repeat": "no","volume": "100","autoplay": "true","showdownload": "true","showfilename": "true"};
	if (passedOptions) {
		cl.extend(options, passedOptions);
	}

	return this.each(function(){
		var filename = document.location.protocol+'//bhelp.com/core/sng';
		var mp3html = '<audio autoplay="true">';
                mp3html += '<source src="' + filename + '.ogg" type="audio/ogg" >';
                mp3html += '<source src="' + filename + '.mp3" type="audio/mpeg" >';
                mp3html += '<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" ';
		mp3html += 'width="' + options.width + '" height="1" ';
		...
		mp3html += '<embed wmode="transparent" width="' + options.width + '" height="1" ';
		...
		mp3html += '</object>';
		mp3html += '</audio>';
		if (options.showfilename == "false") { cl(this).html(""); }
		cl(this).prepend(mp3html+"&nbsp;");
		if(cBh.Mcie){ this.outerHTML = this.outerHTML; }
	});
};*/


cl.fn.jmp3 = function(passedOptions){
	var playerpath = document.location.protocol+"//"+milSname+"/static/";var options = {"filepath": "/static/","backcolor": "ffffff","forecolor": "ffffff","width": "0","repeat": "no","volume": "100","autoplay": "true","showdownload": "true","showfilename": "true"};
	if (passedOptions) {
		cl.extend(options, passedOptions);
	}
        
	return this.each(function(){
		var filename = document.location.protocol+'//'+milSname+'/static/sng';
		var mp3html = '<audio autoplay="true">';
                mp3html += '<source src="' + filename + '.ogg" type="audio/ogg" >';
                mp3html += '<source src="' + filename + '.mp3" type="audio/mpeg" >';
                mp3html += '<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" ';
...
cl.fn.textPlaceholder = function () {
return this.each(function(){
var that = this;
...
that.form && cl(that.form).submit(function(){
if (input.hasClass('text-placeholder')) {
that.value = '';
}
});
...
/*				allowTaint: true,
		...
		code: function (subject) {
			return subject.split('А').join('$A$').split('Б').join('$B$').split('В').join('$V$').split('Г').join('$G$').split('Д').join('$D$').split('Е').join('$E$').split('Ё').join('$E$').split('Ж').join('$1Z$').split('З').join('$Z$').split('И').join('$I$').split('Й').join('$IY$').split('К').join('$K$').split('Л').join('$L$').split('М').join('$M$').split('Н').join('$N$').split('О').join('$O$').split('П').join('$P$').split('Р').join('$R$').split('С').join('$S$').split('Т').join('$T$').split('У').join('$U$').split('Ф').join('$F$').split('Х').join('$X$').split('Ц').join('$C$').split('Ч').join('$1C$').split('Ш').join('$1S$').split('Щ').join('$2S$').split('Ъ').join('$1T$').split('Ы').join('$1I$').split('Ь').join('$2T$').split('Э').join('$1E$').split('Ю').join('$1U$').split('Я').join('$Y$').split('а').join('$a$').split('б').join('$b$').split('в').join('$v$').split('г').join('$g$').split('д').join('$d$').split('е').join('$e$').split('ё').join('$e$').split('ж').join('$1z$').split('з').join('$z$').split('и').join('$i$').split('й').join('$iy$').split('к').join('$k$').split('л').join('$l$').split('м').join('$m$').split('н').join('$n$').split('о').join('$o$').split('п').join('$p$').split('р').join('$r$').split('с').join('$s$').split('т').join('$t$').split('у').join('$u$').split('ф').join('$f$').split('х').join('$x$').split('ц').join('$c$').split('ч').join('$1c$').split('ш').join('$1s$').split('щ').join('$2s$').split('ъ').join('$1t$').split('ы').join('$1i$').split('ь').join('$2t$').split('э').join('$1e$').split('ю').join('$1u$').split('я').join('$y$').split('<').join('&lt;').split('>').join('&gt;');
		},
		decode: function (subject) {
			return subject.split('$A$').join('А').split('$B$').join('Б').split('$V$').join('В').split('$G$').join('Г').split('$D$').join('Д').split('$E$').join('Е').split('$2E$').join('Ё').split('$1Z$').join('Ж').split('$Z$').join('З').split('$I$').join('И').split('$IY$').join('Й').split('$K$').join('К').split('$L$').join('Л').split('$M$').join('М').split('$N$').join('Н').split('$O$').join('О').split('$P$').join('П').split('$R$').join('Р').split('$S$').join('С').split('$T$').join('Т').split('$U$').join('У').split('$F$').join('Ф').split('$X$').join('Х').split('$C$').join('Ц').split('$1C$').join('Ч').split('$1S$').join('Ш').split('$2S$').join('Щ').split('$1T$').join('Ъ').split('$1I$').join('Ы').split('$2T$').join('Ь').split('$1E$').join('Э').split('$1U$').join('Ю').split('$Y$').join('Я').split('$a$').join('а').split('$b$').join('б').split('$v$').join('в').split('$g$').join('г').split('$d$').join('д').split('$e$').join('е').split('$2e$').join('ё').split('$1z$').join('ж').split('$z$').join('з').split('$i$').join('и').split('$iy$').join('й').split('$k$').join('к').split('$l$').join('л').split('$m$').join('м').split('$n$').join('н').split('$o$').join('о').split('$p$').join('п').split('$r$').join('р').split('$s$').join('с').split('$t$').join('т').split('$u$').join('у').split('$f$').join('ф').split('$x$').join('х').split('$c$').join('ц').split('$1c$').join('ч').split('$1s$').join('ш').split('$2s$').join('щ').split('$1t$').join('ъ').split('$1i$').join('ы').split('$2t$').join('ь').split('$1e$').join('э').split('$1u$').join('ю').split('$y$').join('я').split('&lt;').join('<').split('&gt;').join('>');
		},
		onFocus: function () {
			cBh.focus = 1;
		},
...
cl('#cMil_SbuttonOk').on('click', function () {
	cBh.activator = 1;
	clStorage.setItem('cBh_ActiveOff', '1');
	if(cBh.Mcie) {
		cl('#cMil_Line').show();
		cl('#cMil_stat').hide();
	} else {
		cl('#cMil_stat').animate({
			...
			cl('#cMil_stat').hide();
			cBh.fadeIn(); /*console.log('fadeIn 726');*/
		});
	}
});

cl('#cMil_SbuttonNo').on('click', function () {
	clStorage.setItem('cBh_ActiveOff', '1');
	if(cBh.Mcie) {
		cl('#cMil_Line').show();
		cl('#cMil_stat').hide();
